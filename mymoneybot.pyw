...
프로그램정보 = [
    ...
    ['2018-08-28','메뉴중 종목검색, e종목검색 수정'],
    ['2018-09-04','Prbot1857v1, Prbot1857v2에서 한번에 다 매수가 이루어지는 경우 P==None 수정'],
    ['2018-10-04','클릭으로 포트종목 삭제시 금일매도종목에 등재'],
    ['2018-12-01','Query 호출시간 제한함수 추가']
]
...
Ui_업종별종목정보, QtBaseClass_업종별종목정보 = uic.loadUiType(UI_DIR+"업종별종목정보.ui")
class 화면_업종별종목정보(QDialog, Ui_업종별종목정보):
    def __init__(self, parent=None):
        super(화면_업종별종목정보, self).__init__(parent)
        self.setAttribute(Qt.WA_DeleteOnClose)
        self.setupUi(self)
        ...
        self.columns = ['업종코드','종목명','현재가','전일대비구분','전일대비','등락율','누적거래량','시가','고가','저가','소진율','베타계수','PER','외인순매수','기관순매수','거래증가율','종목코드','시가총액','거래대금']
    ...
    def OnReceiveData(self, szTrCode, result):
        ...
        if szTrCode == 't1516':
            ...
            if len(df) == 40:
                self.XQ.Query(업종코드=self.업종코드,종목코드=self.df['종목코드'].values[-1], 연속조회=True)
            else:
                self.model.update(self.df)
                for i in range(len(self.df.columns)):
                    self.tableView.resizeColumnToContents(i)
                ...
                with sqlite3.connect(DATABASE) as conn:
                    cursor = conn.cursor()
                    query = "insert or replace into 업종별종목(업종코드,종목명,현재가,전일대비구분,전일대비,등락율,누적거래량,시가,고가,저가,소진율,베타계수,PER,외인순매수,기관순매수,거래증가율,종목코드,시가총액,거래대금) values(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?)"
                    cursor.executemany(query, self.df.values.tolist())
                    conn.commit()

    def inquiry(self):
        self.업종코드 = self.comboBox.currentText()[:3]
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.robot_columns = ['Robot타입', 'Robot명', 'RobotID', '실행상태', '포트수']
        self.portfolio_columns = ['종목코드', '종목명', 'TAG', '매수가', '수량', '매수일']

        self.robot_model = PandasModel()
        self.tableView_robot.setModel(self.robot_model)
        ...
        df = DataFrame(columns=self.portfolio_columns)
    ...
    def OnClockTick(self):
        ...
        if current.second == 0: # 매 0초
            ...
            if current_str[:2] in _TIME and current_str[3:] in ['00:00', '30:00']:
                ToTelegram("%s : [eBEST API] 로봇 %s개가 실행중입니다. ([%s])" % (current_str, len(_temp), ','.join(_temp)))
    ...
    def LoadRobots(self):
        with open('mymoneybot.robot', 'rb') as handle:
            try:
                ...
                print('OnQApplicationStarted %s' % get_linenumber(), e)
            finally:
                pass
    ...
    def robot_selected(self, QModelIndex):
        Robot타입 = self.robot_model._data[QModelIndex.row():QModelIndex.row()+1]['Robot타입'].values[0]

        uuid = self.robot_model._data[QModelIndex.row():QModelIndex.row()+1]['RobotID'].values[0]
        portfolio = None
        for r in self.robots:
            if r.UUID == uuid:
    ...
    def portfolio_double_clicked(self, QModelIndex):
        RobotUUID = self.robot_model._data[self.tableView_robot_current_index.row():self.tableView_robot_current_index.row() + 1]['RobotID'].values[0]
    ...
    def RobotView(self):
        ...
        self.robot_model.update(DataFrame(data=result, columns=self.robot_columns))
    ...
    def RobotEdit(self, QModelIndex):
        Robot타입 = self.robot_model._data[QModelIndex.row():QModelIndex.row()+1]['Robot타입'].values[0]
        RobotUUID = self.robot_model._data[QModelIndex.row():QModelIndex.row()+1]['RobotID'].values[0]
        ...
        if _action == "actionStocksInIndex":
            if self.dialog.get('업종별종목정보') is not None:
                try:
                    ...
                    self.dialog['업종별종목정보'] = 화면_업종별종목정보(parent=self)
                    self.dialog['업종별종목정보'].show()
            else:
                self.dialog['업종별종목정보'] = 화면_업종별종목정보(parent=self)
                self.dialog['업종별종목정보'].show()
